...
window.checkCanvas = () => {
  ...
  canvases.forEach((cvs, i) => {
    ...
    if (userPixels < 30) {
      // Diturunin dari 100 ke 30 biar gak dibilang kosong
      empty = true;
    } else if (strayPixels / userPixels > 0.45) {
      // Dinaikin dikit biar sistem agak toleran sama coretan meleset tipis
      messy = true;
    } else if (coveredPixels / templatePixels < 0.08) {
      // DIPERKECIL DRASTIS dari 0.25 ke 0.08! Karena pena cuma 2px, 8% ketutup aja udah dianggap rapi.
      incomplete = true;
    }
  });
  ...
  if (empty) {
    Swal.fire({
      ...
      text: "Wonten pasangan ingkang dereng sampeyan tebali. Ayoo diselesaino!",
      confirmButtonColor: "#795548",
      customClass: { popup: "swal-paper", confirmButton: "swal-paper-confirm" },
    });
  } else if (messy) {
    ...
    Swal.fire({
      icon: "warning",
      title: "Kurang Pas!",
      text: "Coretane durung nutupi bentuk pasangan. Coba ditebali kabeh ojo sepotong!",
      confirmButtonColor: "#795548",
      customClass: { popup: "swal-paper", confirmButton: "swal-paper-confirm" },
    });
  } else {
